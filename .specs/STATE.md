...
- **Blockers**:
  ...
  - **Login para as capturas de T23–T27 é do usuário.** O agente não digita senha em formulário, então não há usuário de teste a criar para esse fim (decisão do usuário, 2026-09-20). Quando a fase de UI retomar, o usuário deixa a sessão autenticada no Chrome — uma vez como admin/gestor e uma vez como corretor, para os cenários de papel de T25 — e o agente navega e captura a partir dela. O agente ainda semeia os documentos nos estados necessários pelo banco. A extensão do Chrome está conectada e é o caminho de captura, nunca o painel embutido.
