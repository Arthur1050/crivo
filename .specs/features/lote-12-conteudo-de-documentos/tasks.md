...
*Reprodutibilidade do workflow-as-code.* Regenerar os workflows nesta máquina (Windows, `core.autocrlf=true`) mudava o `jsCode`: o checkout grava `n8n/src/*.mjs` com CRLF e o inliner embutia o ``. O publicado estava certo (saiu de fontes LF), mas o próximo a regenerar publicaria outro hash. `scripts/n8n-inline.mjs` agora normaliza CRLF antes de inlinar; teste novo em `scripts/__tests__/n8n-inline.test.ts` (a mutação que remove a normalização o derruba). Depois da correção, regenerar reproduz exatamente os hashes publicados (`e40331e2…` no principal, `c970a3a9…` no benchmark). Suíte completa de novo, sozinha: **115 arquivos, 1.848 testes**.
