...
| Reação a saída inválida | Regenerar com feedback do motivo, no máximo **2 tentativas**, depois cair no fallback de esclarecimento que já existe | Mesma disciplina do retry atual (2 tentativas), agora aplicada também às barreiras de persona, não só ao schema | n (discretion) |
...
4. WHEN o validador rejeita uma saída THEN o fluxo SHALL regenerar informando ao modelo o motivo da rejeição, no máximo 2 vezes, e SHALL cair no fallback de esclarecimento se a terceira tentativa também for rejeitada.
...
- IF o nó AI Agent atinge `maxIterations` sem concluir THEN o fluxo SHALL enviar o fallback de esclarecimento ao lead, nunca terminar em silêncio.
