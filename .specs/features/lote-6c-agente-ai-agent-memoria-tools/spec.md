...
- [x] Nenhum efeito colateral ocorre sem validação determinística — provado pelo Verifier independente (2026-08-15): sensor de mutação sobre `phase.mjs`/`voice.mjs`/`session.mjs` (5/5 mutações mortas, worktree isolado) + checagem estrutural ao vivo do risco nomeado no `design.md` (`leadId` nunca vindo de `$fromAI`, confirmado na instância publicada via `get_workflow_details`). Relatório: `validation.md`.
